...
def plot_percentage_histogram(data1, data2, subject, year1, year2):
    ...
    đồng thời hiển thị tỷ lệ phần trăm của mỗi khoảng điểm.
    ...
    counts1, bins1, patches1 = axs[0].hist(data1[subject].dropna(), bins=10, edgecolor='black', color='skyblue', density=True)
    counts1 = counts1 * (bins1[1] - bins1[0]) * 100  # Chuyển đổi tỷ lệ phần trăm
    for count, patch in zip(counts1, patches1):
        height = patch.get_height()
        axs[0].text(patch.get_x() + patch.get_width() / 2., height, f'{count:.1f}%', ha='center', va='bottom')
    ...
    counts2, bins2, patches2 = axs[1].hist(data2[subject].dropna(), bins=10, edgecolor='black', color='lightcoral', density=True)
    counts2 = counts2 * (bins2[1] - bins2[0]) * 100  # Chuyển đổi tỷ lệ phần trăm
    for count, patch in zip(counts2, patches2):
        height = patch.get_height()
        axs[1].text(patch.get_x() + patch.get_width() / 2., height, f'{count:.1f}%', ha='center', va='bottom')
    axs[1].set_title(f'Năm {year2}')
    axs[1].set_xlabel('Điểm')

    # Thêm tiêu đề tổng thể cho hình
    plt.suptitle(f'So Sánh Phân Bố Điểm Môn {subject.capitalize()} Giữa Các Năm', fontsize=16, y=1)
